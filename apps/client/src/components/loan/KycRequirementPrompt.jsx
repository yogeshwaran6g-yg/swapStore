...
export const KycRequirementPrompt = ({ status }) => {
  return (
    <div className="bg-white rounded-2xl border border-[#FF8C00]/20 shadow-sm p-8 text-center animate-fade-in">
      <div className="mx-auto w-16 h-16 bg-red-50 rounded-full flex items-center justify-center mb-6">
        <svg className="w-8 h-8 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
      </div>
      <h2 className="text-2xl font-bold text-[#1E293B] mb-4">KYC Verification Required</h2>
      ...
      {status !== 'pending' && status !== 'submitted' && (
        <Link
          to="/profile"
          className="inline-flex justify-center items-center py-3 px-6 rounded-xl shadow-sm text-sm font-bold text-white bg-gradient-to-r from-[#FF8C00] to-[#FF4500] hover:opacity-90 transition-all"
        >
          Go to Profile
        </Link>
      )}
