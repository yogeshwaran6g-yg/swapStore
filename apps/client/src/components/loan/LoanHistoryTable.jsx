...
export const LoanHistoryTable = () => {
  ...
  return (
    <div className="mt-8 bg-white border border-[#FF8C00]/20 rounded-[2rem] p-8 shadow-sm relative">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <h3 className="text-xl font-bold text-[#1E293B] tracking-wide">Your Loan History</h3>
          {loans.length > 0 && (
            <span className="text-xs text-[#475569]">{loans.length} total</span>
          )}
        </div>
        <div className="flex items-center gap-4">
          <button
            onClick={() => refetch()}
            disabled={isFetching}
            className="text-xs text-[#475569] hover:text-[#FF4500] transition-colors disabled:opacity-50 font-medium"
          >
            Refresh
          </button>
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-[#475569]">{isFetching ? 'Syncing...' : 'Live'}</span>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          <div className="overflow-x-auto rounded-xl border border-[#FF8C00]/10 bg-[#FFF5ED]">
            <table className="w-full text-left text-sm text-[#475569]">
              <thead className="text-xs uppercase bg-[#FF8C00]/10 border-b border-[#FF8C00]/10 text-[#1E293B]">
                <tr>
              ...
              <tbody className="divide-y divide-[#FF8C00]/10">
                    ...
                    >
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          <svg className={`w-3.5 h-3.5 transform transition-transform ${expandedLoanId === loan.loan_id ? 'rotate-90 text-[#FF4500]' : 'text-[#475569]'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                          </svg>
                          <span className="font-mono text-[#1E293B] text-xs tracking-wider">{loan.loan_id?.substring(0, 8)}...</span>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <span className="text-[#1E293B] font-bold text-sm">${Number(loan.principal_amount).toLocaleString()}</span>
                      </td>
                      <td className="px-4 py-4">
                        <span className="text-amber-500 font-bold text-sm">${Number(loan.outstanding_principal).toLocaleString()}</span>
                      </td>
                      <td className="px-4 py-4">
                        <span className="text-[#475569] text-sm">${Number(loan.total_interest_paid || 0).toLocaleString()}</span>
                      </td>
                      <td className="px-4 py-4">
                        {loan.is_overdue ? (
                    ...
                    {expandedLoanId === loan.loan_id && (
                      <tr>
                        <td colSpan="7" className="p-0">
                          <div className="bg-[#FFF5ED] p-6 border-t border-[#FF8C00]/10">
                            <h4 className="text-amber-500 font-semibold mb-4 text-xs uppercase tracking-wider">Interest Ledger History</h4>
                            {loan.ledger && loan.ledger.length > 0 ? (
                              <div className="overflow-x-auto rounded-lg border border-[#FF8C00]/10 bg-white">
                                <table className="w-full text-left text-xs text-[#475569]">
                                  <thead className="text-[10px] uppercase bg-[#FF8C00]/5 border-b border-[#FF8C00]/10 text-[#1E293B]">
                                    <tr>
                                  ...
                                  <tbody className="divide-y divide-[#FF8C00]/5">
                                    {loan.ledger.map((entry) => (
                                      <tr key={entry.id} className="hover:bg-[#FFF5ED]">
                                        ...
                                        <td className="px-3 py-2.5 font-mono text-[#1E293B] font-bold">
                              ...
                              <p className="text-[#475569] text-xs">No interest ledger records generated for this loan yet.</p>
                            )}
                          </div>
                        </td>
                      </tr>
            ...
            <div className="flex items-center justify-between mt-5 px-1">
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                  currentPage === 1
                    ? 'bg-[#FFF5ED] text-[#475569] cursor-not-allowed border border-[#FF8C00]/10'
                    : 'bg-white text-[#1E293B] hover:bg-[#FFF5ED] border border-[#FF8C00]/20 active:scale-95'
                }`}
              >
        ...
        <div className="text-center py-12">
          <svg className="mx-auto h-12 w-12 text-[#475569]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
          <h3 className="mt-4 text-lg font-medium text-[#1E293B]">No loans found</h3>
          <p className="mt-1 text-[#475569] text-sm">Get started by requesting a loan above.</p>
        </div>
      )}
    </div>
  );
